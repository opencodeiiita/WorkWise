export default (props)=>{
    ...
    return (
        <div className="h-10 text-[.5em] relative flex items-center text-gray-400">
            ...
            <label className={`${localStorage.getItem(props.keyno)==null?"hidden":""} flex`}>
            <input type="radio" className={`cursor-pointer 
            opacity-100 w-0`} name="projects"
            onFocus={select}
            onBlur={deselect}
            />
                <div className="cursor-pointer px-3 py-1 rounded-xl transition hover:bg-gray-200 hover:text-gray-800">
