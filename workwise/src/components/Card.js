        ...
        <div className="w-72 m-4 h-max bg-gray-50 rounded-lg overflow-hidden">
            <div className="flex justify-start ml-4 mt-4 mb-2">
                {card.tags.map((item,index) => {
                    ...
                    if(item==='Design'){
                        return (<span key={index} class="inline-block bg-violet-700 rounded px-3 py-1 text-sm font-semibold text-gray-50 mr-2 mb-2">{item}</span>);
                    }
                    if(item==='Content'){
                        return (<span key={index} class="inline-block bg-amber-500 rounded px-3 py-1 text-sm font-semibold text-gray-50 mr-2 mb-2">{item}</span>);
                    }
                    if(item==='Planning'){
                        return (<span key={index} class="inline-block bg-orange-700 rounded px-3 py-1 text-sm font-semibold text-gray-50 mr-2 mb-2">{item}</span>);
                    }
                })}
            </div>
            {card.imgUrl &&
                <div className="flex w-full h-28 justify-center">
                    <img className="w-64 h-32 rounded" src={card.imgUrl} alt="" />
                </div>
            }
            {card.imgUrl &&
                <div className="px-5 pt-4 mt-4">
                    <div className="font-medium font-body text-base mb-1">{card.heading}</div>
                    <p className="text-gray-500 font-body text-base">{card.desc}</p>
                </div>
            }
            {!card.imgUrl &&
                <div className="px-5">
                    <div className="font-medium font-body text-base mb-1">{card.heading}</div>
                    <p className="font-body text-gray-500 text-base">{card.desc}</p>
                </div>
            }
